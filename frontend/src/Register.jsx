...
const handling=(e,value2)=>{
const gh=e.target.value
newdetails((prev)=>({...prev,[value2]:gh}))
}
const handleregister=async()=>{
  const response=await fetch('http://localhost:1000/home/register',{
    method:"POST",
    headers:{
...
if(response.ok){
  console.log(gh.details[0].token)
storeinlocalstorage(gh.details[0].token)
toast.success('registration successfully')
navigate('/')

}
if(!response.ok){
  const op=gh.message||gh
  if(gh.username){
    toast.error(gh.username)
  }
  if(gh.email){
    toast.error(gh.email)
  }
  if(gh.phone){
    toast.error(gh.phone)
  }
  if(gh.password){
    toast.error(gh.password)
  }
  if(gh.mssg){
    toast.error('phone must be of numeric digits')
  }
